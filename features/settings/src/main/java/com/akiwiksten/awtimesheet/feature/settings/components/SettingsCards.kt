...
internal fun SettingsGlobalDefaultsCard(state: SettingsContentBodyState) {
    SettingsCard {
        ...
        Row(
            ...
            Text(
                text = stringResource(id = R.string.enable_test_features),
                style = MaterialTheme.typography.bodyLarge
            )
            Switch(
                checked = state.uiState.data.enableTestFeatures,
                onCheckedChange = state.actions.onEnableTestFeaturesChange
            )
        }
    }
}
